...
def choose_backend(choose_end):
    ...
    if choose_end == "1":
        ...
        backend = "http://kerryji.japaneast.cloudapp.azure.com:25500/sub?"
    else:
        backend = choose_end
    print("\n此次转换使用的后端为：" + backend)
...
def high_mod():
    ...
    insert = "true"
    udp = "true"
    result = backend + 'target=' + target + "&url=" + url + "&insert=" + insert + "&config=" + config +\
         "&emoji=" + emoji + "&tfo=" + tfo + "&expand=" + expand + "&scv=" + scv + \
        "&udp=" + udp + "&fdn=" + fdn + "&sort=" + sort + "&new_name=" + new_name
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sub_urls', "-s", type=str)
